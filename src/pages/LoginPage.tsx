...
const views: Record<string, React.FC<any>> = {
    ...
    verify: (props: any) => <VerifyCodeForm {...props} />,
    forgot: (props: any) => <ForgotPasswordForm {...props} />,
    reset: (props: any) => <ResetPasswordForm {...props} />,
}
...
export default function LoginPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const withBackdrop = results.find((movie: any) => movie.backdrop_path)
            const backdrop = withBackdrop
                ? `${BACKDROP_BASE_URL}${withBackdrop.backdrop_path}`
                : fondo
            const trending = results.filter((movie: any) => movie.poster_path).slice(0, 6)
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="relative w-full max-w-sm backdrop-blur-md bg-white/10 p-8 rounded-xl border border-white/20 shadow-2xl">
                        ...
                        {(() => {
                            ...
                            return ViewComponent ? <ViewComponent {...viewProps[currentView]} /> : null
                        })()}
                    </div>
                </div>
            )}
