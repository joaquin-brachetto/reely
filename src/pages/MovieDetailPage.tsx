...
export default function MovieDetailPage() {
    ...
    const certification = getCertification(movie, mediaType as 'movie' | 'tv', country || 'US')
            ...
            <div className="relative z-10 max-w-5xl mx-auto p-8">
                ...
                <div className="flex flex-col sm:flex-row gap-8">
                    <div className="relative self-center sm:self-start shrink-0">
                        ...
                        <button
                            onClick={() => user
                                ? toggle({ mediaType: mediaType as 'movie' | 'tv', tmdbId: Number(id), title, posterPath: movie.poster_path, releaseDate })
                                : navigate('/login')}
                            title={!user
                                ? 'Iniciá sesión para guardar en tu watchlist'
                                : isSaved(mediaType as 'movie' | 'tv', Number(id)) ? 'Quitar de mi watchlist' : 'Guardar en mi watchlist'}
                            className="absolute top-2 right-2 bg-black/60 hover:bg-black/80 rounded-full p-1.5 transition"
                        >
                            <Bookmark
                                className="w-4 h-4"
                                fill={user && isSaved(mediaType as 'movie' | 'tv', Number(id)) ? '#22c55e' : 'none'}
                                color={user && isSaved(mediaType as 'movie' | 'tv', Number(id)) ? '#22c55e' : 'white'}
                            />
                        </button>
                    </div>
                                ...
                                providers[key] && (
                                    <div key={key} className="mb-4">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {providers[key].map((provider: any) => (
                                                <img
